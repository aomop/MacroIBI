...
params:
  ...
  report_date: "Default Generation Date"
  data: NA
...
# **IBI Overview**
This report provides a summary of the biotic integrity of `r params$user_title`, based on macroinvertebrate organisms collected on `r { d <- params$user_date; add_ordinal_suffix(if (inherits(d, "Date")) d else parse_flexible_date(as.character(d))) }`. The Index of Biotic Integrity (IBI) method seeks to quantify
...
has_comparison <- !is.null(params$comparison_metrics) &&
  is.data.frame(params$comparison_metrics) &&
  nrow(params$comparison_metrics) > 0
```
...
if(!is.null(total_score) && !is.na(total_score)) {
  if(total_score >= 38) {
    cat("A total IBI score of ", round(total_score, 2), " suggests minimal impariment; this wetland is in excellent condition. The high score reflects a robust and diverse macroinvertebrate community,
        suggesting high water quality and ecological integrity.\n\n")
    } else if(total_score >= 28) {
      cat("A total IBI score of ", round(total_score, 2), " suggests minor impairment; this wetland is in good condition. The macroinvertebrate community is relatively diverse, though there are some
          signs of ecological stress that could be related to habitat degradation or pollution.\n\n")
    } else if(total_score >= 20) {
      cat("A total IBI score of ", round(total_score, 2), " indicates some impairment; this wetland is in moderate condition. The macroinvertebrate community has lost some diversity and sensitivity,
          but clearer signs of ecological stress are likely not present.\n\n")
    } else if(total_score >= 10) {
      cat("A total IBI score of ", round(total_score, 2), " suggests significant impairment; this wetland is in poor condition. This low score highlights notable ecological degradation and suggests              that the wetland is considerably impaired, potentially from pollutants or habitat disruption.\n\n")
    } else if(total_score < 10) {
...
if(!is.null(snail_taxa) && !is.na(snail_taxa)) {
  if(snail_taxa >= 6.5) {
    ...
    cat("- Snail Taxa: A diversity score of ", round(snail_taxa, 2), " implies no gastropod taxa were found in the sample. This absence could indicate significant impairment or environmental stress.\n\n")
  }
} else {
...
`r if (has_comparison) paste0("The boxplots in figure 1 compare the metric scores from ", params$user_title, " with scores collected from other wetlands by the natural resources department. The NRD makes an effort to conduct all IBI surveys with the same temporal and spatial conditions to ensure meaningful comparisons.") else "*(Comparison data is not available for this report.)*"`

```{r comparison-boxplot, warning = FALSE, echo = FALSE}
if (has_comparison) {
  comparison_other <- subset(params$comparison_metrics,
                             metric_name != "IBI Score (0-50)")
  current_other    <- subset(params$data,
                             metric_name != "IBI Score (0-50)")

  figure1_text <- escape_latex(paste0(
    "Distributions of calculated metric scores from all surveyed sites. ",
    "Red lines indicate scores from ", params$user_title
  ))
  ...
  comp_boxes <- ggplot(
    ...
    geom_segment(
      ...
      aes(
        x    = as.numeric(factor(
                 .data$metric_name,
                 levels = unique(comparison_other$metric_name)
               )) - 0.4,
        xend = as.numeric(factor(
                 .data$metric_name,
                 levels = unique(comparison_other$metric_name)
               )) + 0.4,
        y    = .data$adj_score,
...
if (!has_comparison) {
  cat("Comparison data is not available for this report.\n\n")
} else {
  ...
  metric_summary <- merge(
    ...
    dplyr::mutate(
      metric_name = gsub(": ", "", .data$metric_name)
    )

  higher_metrics <- metric_summary$metric_name[
    metric_summary$adj_score_current >
      metric_summary$adj_score_median * 1.25
  ]

  lower_metrics <- metric_summary$metric_name[
    metric_summary$adj_score_current <
      metric_summary$adj_score_median * 0.75
  ]

  general_comparison <- median(
    metric_summary$adj_score_current /
      metric_summary$adj_score_median,
    na.rm = TRUE
  )

  however <- FALSE

  if (!is.na(general_comparison) && general_comparison >= 1.25) {
    cat("The metric scores for this wetland are generally higher than ",
        "those of other sampled wetlands, indicating relatively better ",
        "conditions overall.\n\n", sep = "")
  } else if (!is.na(general_comparison) && general_comparison <= 0.75) {
    cat("The metric scores for this wetland are generally lower than ",
        "those of other sampled wetlands, suggesting relatively poorer ",
        "conditions or higher impairment.\n\n", sep = "")
  } else if (!is.na(general_comparison)) {
    cat("The metric scores for this wetland are generally similar to ",
        "other sampled wetlands.\n\n", sep = "")
  }

  if (length(higher_metrics) > 0 & !is.na(general_comparison) & general_comparison <= 0.75) {
    cat("However, these metrics were notably higher than average: ",
        paste(higher_metrics, collapse = ", "), ".\n\n", sep = "")
    however <- TRUE
  }

  if (length(higher_metrics) > 0 & !is.na(general_comparison) & general_comparison >= 1.25 &
      however == FALSE ||
      length(higher_metrics) > 0 & !is.na(general_comparison) &
      abs(general_comparison - 1) <= 0.1 & however == FALSE) {
    cat("These metrics were notably higher than average: ",
        paste(higher_metrics, collapse = ", "), ".\n\n", sep = "")
    however <- TRUE
  }

  if (length(lower_metrics) > 0 & !is.na(general_comparison) & general_comparison <= 0.75 &
      however == FALSE) {
    cat("These metrics were notably lower than average: ",
        paste(lower_metrics, collapse = ", "), ".\n\n", sep = "")
    however <- TRUE
  }

  if (length(lower_metrics) > 0 & !is.na(general_comparison) & general_comparison >= 1.25 &
      however == FALSE ||
      length(lower_metrics) > 0 & !is.na(general_comparison) &
      abs(general_comparison - 1) <= 0.1 & however == FALSE) {
    cat("However, these metrics were notably lower than average: ",
        paste(lower_metrics, collapse = ", "), ".\n\n", sep = "")
  }
  ...
  if (length(lower_metrics) > 0 & !is.na(general_comparison) & general_comparison >= 1.25 &
      however == TRUE ||
      length(lower_metrics) > 0 & !is.na(general_comparison) &
      abs(general_comparison - 1) <= 0.1 & however == TRUE) {
    cat("Additionally, these metrics were notably lower than average: ",
        paste(lower_metrics, collapse = ", "), ".\n\n", sep = "")
  }

  if (length(higher_metrics) == 0 && length(lower_metrics) == 0) {
    cat("There are no metrics with notably higher or lower scores ",
...
`r if (!has_comparison) "*(Comparison data is not available for this report.)*"`

`r if (has_comparison) paste0("The boxplot in figure 2 compares the total IBI score from ", params$user_title, " with other wetlands surveyed by the natural resources department.")`

```{r total-boxplot, warning = FALSE, echo = FALSE}
if (has_comparison) {
  comparison_total <- subset(params$comparison_metrics,
                             metric_name == "IBI Score (0-50)")
  current_total    <- subset(params$data,
                             metric_name == "IBI Score (0-50)")

  figure2_text <- escape_latex(paste0(
    "Distribution of total IBI scores from all surveyed sites. ",
    "The red line indicates the total score for ", params$user_title
  ))
...
if (has_comparison) {
  ...
  if (!is.na(diff_tot) && diff_tot < -0.1) {
  ...
  if (!is.na(diff_tot) && diff_tot >= -0.1 & diff_tot <= 0.1) {
