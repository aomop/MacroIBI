...
- **PNG table image export not working** --- The PNG export requires Google
  Chrome or Chromium. Make sure Chrome is installed on your computer. If
  Chrome is installed but the export still fails, run
  `chromote::find_chrome()` in the Console to confirm R can find it.
- **PDF report export not working** --- PDF generation requires a LaTeX
  installation. Install TinyTeX (a lightweight LaTeX distribution) by
  running these two lines in the Console:
  ```r
  install.packages("tinytex")
  tinytex::install_tinytex()
  ```
  This is a one-time setup step. Restart RStudio after installation
  completes.
...
Two sets of downloads are available:

**Data tab sidebar:**

- **Raw Data CSV** --- For archiving or re-uploading into the app. Use only
  CSVs exported from MacroIBI without external edits.

**Results tab:**

- **Results CSV** --- Final calculated metric scores.
- **Table Image (PNG)** --- Formatted metric table with titles and dates.
  Requires Chrome or Chromium.
- **Data Summary (PDF)** --- Concise one-page snapshot of metrics. Requires
  LaTeX (see Troubleshooting above).
- **Full Report (PDF)** --- Comprehensive report including current metrics and
  comparisons to other autosaved sessions. Requires LaTeX.
